...
**Инструмент раскрашен в Gruvbox.** Область, помеченная `class="tool"` — док, переключатели в верхней строке, легенда под машиной, — в обеих системных темах показана в палитре Gruvbox, а страница остаётся в палитре токенов.

Схему из примера `review` правят в `machjs-editor` и рисуют в `machjs-diagram`, но это документ, а не инструмент, — поэтому она стоит вне помеченной области и сохраняет цвета страницы, а конвейер, который её рецензирует, стоит в доке и раскрашен как инструмент.
