...
`history(sel)` — рекордер из `debug`: он записывает каждое состояние, в котором машина побывала, вместе с контекстом. В стек кладётся не состояние, а его номер в этой записи. Одного прямоугольника не хватило бы: первое перетаскивание началось в `empty`, значит и возвращаться нужно в `empty`, а не в `ready` с выделением 0×0, которому не соответствует ни одно состояние схемы.
...
`jump` не переход: ничего не отправляется, выходное событие не возникает, `TRANSITION` не публикуется (README, «Ограничения»). Поэтому отмена не попадает в собственный стек. О перемещении по записи говорит сам рекордер — событием `moved`, на которое подписана отрисовка страницы:
