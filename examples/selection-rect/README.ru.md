...
Разбор задачи от постановки до готового автомата: прямоугольник выделения в браузере. Разделы идут в порядке работы: граф переходов, контекст, условия и операции, обращение из браузера, анализ. Типы вводятся по мере необходимости.

Обозначения и определения даны в [руководстве](https://github.com/evgkch/machjs/blob/master/packages/core/README.ru.md). Ссылки вида «п. 4.2» ведут на пункты этого документа; на руководство ссылаемся по названию раздела, например «README, „Схема переходов“».
...
Задача: рисовать прямоугольник выделения на экране, перемещать его и менять размер за углы и рёбра. Клавиша Escape отменяет текущее действие, щелчок по пустому месту снимает выделение.

Одно и то же перемещение указателя растягивает новый прямоугольник, переносит существующий или меняет его размер. Что именно произойдёт, зависит от того, что происходило до нажатия и куда попал указатель.
...
В первой схеме нет исполняемого кода: записана только структура состояний и переходов.
...
Три правила в паре `ready` + `down` покрывают три разных места нажатия, а два правила в паре `drawing` + `up` покрывают прямоугольник меньше допустимого размера и все остальные случаи. Чем именно они различаются, пока не записано.
...
Условия из п. 2.3 различают нажатие на ручку и нажатие в середину, а значит читают сам прямоугольник.
...
> [!NOTE]
> Ниже приведён набросок без `satisfies`, и компилятор его не примет: вход в состояние с контекстом требует функции контекста. Полная схема стоит в п. 5.3, вместе с операциями.
...
Событие `draw` содержит прямоугольник, поэтому в его `emit` стоит пара: имя и функция данных (README, «Схема переходов»). Событие `clear` не содержит данных, и в его `emit` стоит одно имя.

Данные для `draw` строит функция `shot` из листинга п. 5.1. Она единственная в примере читает контекст уже после перехода.
...
Координаты округляются один раз, на входе. При масштабировании страницы и на HiDPI-экране `clientX` и ограничивающий прямоугольник дробные, а от целых значений идёт весь дальнейший счёт: контекст, допуск условий и CSS-рамки.
...
Имена ручек повторяют обозначения сторон света, как у курсоров CSS, поэтому значение курсора получается подстановкой имени.
...
Все операции здесь записаны именованными функциями, поэтому `?` в подписях не встречается: форматтер берёт имя у самой функции (README, «Подписи и имена»). У рёбер, ведущих в `empty`, подписи `WITH` нет: это состояние ничего не хранит, и функции контекста у таких переходов нет.
...
Недостижимых состояний в схеме нет, из каждого состояния есть выход, а безусловные правила стоят последними, поэтому мёртвых правил не возникает.
...
Отмена снимает перетаскивание целиком, а не отдельное событие `move`. Встроенная `history` записывает каждый переход, поэтому один её шаг назад вернул бы один отсчёт указателя. Функция `log` передаёт в `sink` объект перехода целиком, поэтому запись в стек можно поставить под условие.
...
Условие читает `source` и `target` одного перехода, поэтому запись появляется только на шаге *внутрь* перетаскивания: по одной записи на перетаскивание, сколько бы событий `move` оно ни заняло.

`history(sel)` строит рекордер из `debug`: он записывает каждое состояние, в котором машина побывала, вместе с контекстом. В стек кладётся не состояние, а его номер в этой записи. Одного прямоугольника не хватило бы: первое перетаскивание началось в `empty`, значит и возвращаться нужно в `empty`, а не в `ready` с выделением 0×0, которому не соответствует ни одно состояние схемы.
...
`jump` не выполняет перехода: ничего не отправляется, выходное событие не возникает, `TRANSITION` не публикуется (README, «Ограничения»). Поэтому отмена не попадает в собственный стек. Перемещение по записи публикует сам рекордер, событием `moved`; на него подписана отрисовка страницы:
...
В доке справа автомат нарисован виджетами [`@evgkch/machjs-inspector`](https://github.com/evgkch/machjs/tree/master/packages/inspector): легенда состояний, диаграмма переходов и прогон. Их связывает `<machjs-desk>`: он подключает виджеты к общему субъекту и добавляет каждому переключатель:
