...
// ── the machine, drawn ────────────────────────────────────────────────────────
//
// The inspector's widgets on the same `sel`, each wired to one subject and given a switch by the
// shell's desk; the widgets follow the machine themselves.
board({
  subject: fromMachine(sel, { history: past }),
  enroll: (desk) => {
