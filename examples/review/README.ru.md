...
Разбор задачи от постановки до готового автомата: изменение проходит ревью — автоматический гейт, затем две подписи, затем выпуск. Документ под ревью — схема конечного автомата: гейт — библиотечные `validate` и `analyze`, а машину, ведущую ревью, рисуют на странице виджеты [`@evgkch/machjs-inspector`](https://github.com/evgkch/machjs/tree/master/packages/inspector). Разделы идут в порядке работы: граф переходов, контекст, условия и операции, гейт, страница, анализ. Типы вводятся по мере необходимости.
...
Подача — не один объект, к которому по ходу добавляются поля: состав контекста **разный в разных состояниях**.
...
Функции данных — это `by`, вторая половина пары `emit`: они читают контекст уже _после_ перехода и превращают его в данные события. Страница строит ленту из этих строк и собственного журнала не ведёт.
...
Две степени строгости `validate` сохраняются: ошибка — блокер, предупреждение показывается рецензентам. `found` переводит замечания в `Fault`, поэтому условие в автомате проверяет одно: есть ли блокер.
...
`paint` срабатывает после каждого перехода и читает автомат — и ничего больше. Состояние — размеченное объединение, поэтому `s.type` сужает `s.context`: внутри ветки `review` подписи в области видимости, а список замечаний — нет, потому что у документа в ревью нет списка замечаний.

Кнопка включена, когда `can(event)` — та же проверка, которую выполнит следующий `dispatch`, — возвращает `OK`; множество доступных действий записано в схеме, а не на странице. Проверка сужается и по данным события: после подписи anna `can("sign", { who: "anna", … })` возвращает `REJECTED`, тот же вызов для boris — `OK`. Опции селектора отключаются той же проверкой; если выбранная опция отключена, выбор переставляется на доступную.
