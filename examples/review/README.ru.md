...
Разбор задачи от постановки до готового автомата: конвейер ревью, который ведёт изменение от черновика до выпуска. Разделы идут в порядке работы: граф переходов, контекст, условия и операции, гейт, страница, анализ. Типы вводятся по мере необходимости.

Обозначения и определения приведены в [руководстве](https://github.com/evgkch/machjs/blob/master/packages/core/README.ru.md). Ссылки вида «п. 4.2» ведут на пункты этого документа; на руководство ссылаемся по названию раздела: «README, «Схема переходов»».
...
Задача: изменение проходит ревью. Сначала его проверяет машина, затем двое из совета трёх рецензентов подписывают, затем изменение выпускают. Подпись электронная: ECDSA над текстом документа. На проверку подаётся схема конечного автомата: библиотека проверяет документы, написанные на её собственном языке, и рисует их.

Такой процесс обычно пишут как одну запись и строку статуса: объект `submission` со всеми полями и поле `status`, указывающее, какие из них сейчас имеют смысл. Эта форма допускает ошибку: документ со статусом `shipped`, но с открытым списком замечаний, или `blocked` с уже стоящей подписью. Запись содержит все поля, а строка статуса их не ограничивает.
...
На входе семь событий: `write` с новым текстом, `submit`, `checked` с ответом гейта, `sign` с именем рецензента и самой подписью, `reject` с именем рецензента и причиной, `ship` и `withdraw`. На выходе два: `gate` с текстом для проверки и `logged` с одной строкой для ленты.
...
Исполняемого кода (функций) в ней пока нет: записана только структура состояний и переходов.
...
Два правила в паре `checking` + `checked` соответствуют двум ответам гейта: пропустить в `review` или отклонить в `blocked`. Два правила в паре `review` + `sign` соответствуют двум случаям подписи: подпись завершает кворум или ещё нет. Чем правила в паре различаются, в схеме пока не записано. У `shipped` правил нет: это конечное состояние.
...
У обеих ошибок одна причина: в ячейке несколько правил, но нет условий, поэтому всегда срабатывает первое (README, «Схема переходов» и «Ограничения»). Предупреждение о `shipped` чинить не нужно: так `validate` отмечает конечное состояние (README, «validate»).
...
Условия из п. 2.3 должны различать, вернул ли гейт блокирующее замечание и завершает ли подпись кворум. Для этого условиям нужны список замечаний гейта и уже поставленные подписи, то есть контекст.
 ...
 * подписи стоят над одним текстом. Любая правка даёт новый текст, и каждый выход из `review`
...
Поля подачи не накапливаются в одном объекте: состав контекста **разный в разных состояниях**.
...
Остальные поля объявлены в контексте конкретной фазы и вне её не существуют.

Единая запись со всеми полями потребовала бы заглушки для каждой фазы без поля: пустой список замечаний, список без подписей, нулевую метку времени. Контекст, привязанный к состоянию, заглушку исключает: у `draft` поля `faults` нет.
...
> [!NOTE]
> Ниже приведён набросок без `satisfies`, компилятор его не примет: вход в состояние с контекстом требует функции контекста, поэтому полная схема стоит в п. 5.3, вместе с операциями. Здесь показано только, где в правиле стоят имена условий.
...
В выводе `validate` ошибок о мёртвых правилах больше нет, а ячейки устроены по-разному. `checking` + `checked` кончается безусловным правилом: у ответа гейта есть исход в любом случае. В `review` + `sign` безусловного правила нет: повторная подпись не подходит ни одному условию, и `dispatch` возвращает `REJECTED`. Отказ от перехода — такой же штатный исход, как переход (README, «validate»). `reject` тоже стоит под условием: запрос правок при действующей подписи противоречил бы ей.
...
/** Нашёл ли гейт что-то блокирующее. Предупреждения не блокируют: их показывают рецензентам. */
...
/** У этого человека нет действующей подписи. На `sign` условие пропускает первую подпись,
...
`clean` читает ответ гейта: при блокере возвращает `false`, при одних предупреждениях `true`. `last` проверяет кворум: подпись не повторная и доводит счёт до `QUORUM`. `unsigned` стоит в двух ячейках. В `sign` это условие пропускает первую подпись рецензента; кворумный случай разобран первым правилом, потому что условия проверяются по порядку. В `reject` то же условие пропускает запрос правок от рецензента без действующей подписи. Рецензенту с действующей подписью `can("sign", …)` возвращает `REJECTED`, и страница отключает его кнопку (п. 7.3). Совет на одного больше кворума, поэтому и в `approved` `reject` доступен хотя бы одному рецензенту. Условия только читают контекст и данные события, не изменяя их (README, «Ограничения»).
...
/** Вход в `review`: предупреждения гейта попадают в `notes`, подписей пока нет. */
function opened(
  c: Ticket,
  faults: readonly Fault[],
): Ticket & { notes: readonly Fault[]; signs: readonly Sign[] } {
 ...
 *
 * Собрано заново, а не через `...c`, по той же причине, что `restarted`: `notes` принадлежит
 * контексту `review`, в контекст `approved` не входит, а `...c` перенёс бы его дальше.
 */
function sealed(
  c: Ticket & { notes: readonly Fault[]; signs: readonly Sign[] },
  p: { who: string; sig: string },
...
/**
 * Запрос правок и его автор образуют контекст `changes`.
 *
 * Тоже собран заново: оба исходных контекста содержат подписи, а контекст `changes` их не
 * содержит. Подписи и так привязаны к тексту под ревью, см. `Sign`.
 */
function asked(
  c: Ticket,
...
`restarted`, `sealed` и `asked` возвращают только перечисленные поля. Вернуть `c` целиком прошло бы типизацию, потому что контекст `review` совместим с `Ticket`. Но тогда подписи попали бы в `draft`, а `notes` в `approved`, хотя в типах этих контекстов таких полей нет.

`fixed` и `addressed` записывают пункт в `closed`, а не удаляют. Исправила ли правка проблему, покажет следующий прогон гейта: неисправленный пункт попадёт в новый список замечаний, рядом со старой записью в `closed`.

Каждая функция возвращает новый объект, а не изменяет переданный (README, «Ограничения»).

### 5.2. Выходные события

Оба выходных события содержат данные, поэтому каждый `emit` записан парой: имя и функция данных (README, «Схема переходов»). Автомат не запускает гейт и не обновляет страницу: он испускает `gate` и `logged`, а обрабатывает их приложение (п. 7.2).
...
/* В каждой строке записан номер раунда: в ленте раунды различаются только по нему. */
...
Функции данных стоят в паре `emit` вторыми, на месте `by`: они читают контекст уже _после_ перехода и превращают его в данные события. Страница строит ленту из этих строк и собственного журнала не ведёт.
...
Автомат создаётся в состоянии `draft` с контекстом `Ticket`, и в поле `doc` записана сама схема: `START`, турникет на языке библиотеки.

## 6. Гейт

Гейт выполняет автоматическую проверку перед ревью. Проверяется схема, поэтому проверки взяты из самой библиотеки: `validate` ищет замечания, `analyze` возвращает факты о графе.
...
/** Схема в том виде, в каком её отдаёт редактор: ключи состояний, значения любые. */
export type Graph = Record<string, unknown>;
```

`Graph` намеренно широкий: функции ниже принимают граф, который может быть бессмыслицей, и возвращают значение, а не бросают исключение. `object` потерял бы имена состояний: `keyof object` равен `never`, и `nodes` вернул бы пустой список.
...
Гейт принимает текст, а не схему: автор отправляет документ, и «текст не читается» — один из ответов гейта. Чтение выполняет `readSchema` инспектора. Он ничего не бросает и принимает обе формы, которые пишут инструменты: язык правил и дамп `JSON.stringify(machine)`. Рецензент может вставить в редактор любую из них.

`read` экспортируется, и страница вызывает его же для рисунка (п. 7.3). Одна функция читает документ и для проверки, и для рисунка: рецензент видит на рисунке то место, на котором гейт остановил документ, вплоть до строки. В поле `rules` записан номер строки для каждого правила: он и связывает текст в редакторе с дугами диаграммы.
...
const found = (graph: Graph, start: string): Fault[] =>
  validate(graph, start)
    // Тупик в список находок не попадает: `validate` помечает его предупреждением, но
    // состояние без выходов обычно и есть задуманный финал. Случай «не работает ничего»
    // разбирают внутренние правила ниже.
...
Две степени строгости `validate` сохраняются: ошибка становится блокером, предупреждение показывается рецензентам. `found` переводит замечания в `Fault`, поэтому условие в автомате проверяет одно: есть ли блокер.
...
Внутренние правила заданы этой организацией, а не библиотекой. Их три: схема без выхода из всех состояний; имя состояния не в нижнем регистре; условие без имени. Третье относится к сериализованной форме: дамп записывает вместо функции её имя, а вместо безымянного условия ставит `?`, и правило проверяет `when === "?"`.

### 6.4. Прогон гейта

```ts
/** Документ, который не читается, даёт одно замечание: анализировать нечего. */
...
В ответе гейта только список замечаний: находки библиотеки, затем находки внутренних правил. Размер схемы (состояния, правила, достижимость) страница вычисляет сама при отрисовке (п. 7.3).

## 7. Обращение из браузера

### 7.1. Разметка и отправка

Страница показывает одну подачу. На сцене документ стоит дважды. Слева редактор инспектора `<machjs-editor>`: он красит каждое состояние в его полосу и отмечает строки правил, которые могут сработать. Справа `<machjs-diagram>`, построенная по тому же чтению. Рядом стоит лист рецензии: раунд, число подписей, по строке на каждого члена совета, открытые замечания и закрытые пункты.
...
/** Кто может подписывать: по кнопке на участника, имя записано в атрибуте `data-sign`. */
const signs = [
  ...document.querySelectorAll<HTMLButtonElement>("[data-sign]"),
].map((button) => [button.dataset["sign"]!, button] as const);
```

Подпись настоящая: ECDSA P-256 через WebCrypto над текстом документа. Ключи создаются при загрузке. WebCrypto асинхронен, а `dispatch` синхронен, поэтому подпись вычисляется в обработчике, до отправки события.

```ts
/** По паре ключей P-256 на участника. Настоящий конвейер взял бы их из хранилища. */
...
`checked` отправляется через `setTimeout`: вложенный `dispatch` библиотека не выполняет (README, «Атомарность и вложенные вызовы»).

Ожидание сделано состоянием автомата. `submit` испускает `gate`, этот код выполняет проверки и отправляет `checked` обратно, а между ними автомат стоит в `checking`. В `checking` нет правила `write`, поэтому документ в это время недоступен для правки. В обработчике подписки нет ни промиса, ни флага, ни переменной `busy`.

### 7.3. Отрисовка

```ts
/** Строка из двух линий: о чём замечание и что в нём записано. */
...
/** Пункт, который подняли и на который ответили: он сохранён и помечен раундом. */
...
function paint(): void {
  ...
  // сработать: это тот же `can`, что и у кнопок.
...
`paint` срабатывает после каждого перехода и читает только автомат. Состояние объявлено размеченным объединением, поэтому `s.type` сужает `s.context`: внутри ветки `review` в области видимости подписи, а списка замечаний там нет, потому что у документа в ревью его нет.

Кнопка включена, когда `can(event)` возвращает `OK`. Это та же проверка, которую выполнит следующий `dispatch`, и множество доступных действий записано в схеме, а не на странице. Проверка сужается и по данным события: после подписи anna `can("sign", { who: "anna", … })` возвращает `REJECTED`, а для boris тот же вызов возвращает `OK`. Опции селектора отключаются той же проверкой; если выбранная опция отключена, выбор переставляется на доступную.
...
Все операции здесь записаны именованными функциями, поэтому `?` в подписях не встречается. `WITH` стоит у каждого правила: у каждого перехода есть функция контекста. После `EMIT` в подписи стоит имя события, а не функции данных, и `by` остаётся единственным словом, которого в диаграмме нет (README, «Подписи и имена»).
...
Недостижимых состояний нет, мёртвых правил нет, и у каждого состояния, кроме одного, есть выход.
