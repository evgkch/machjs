...
Пакет поставляется только в формате ESM. Собственная зависимость одна — `lit`, база custom elements. Peer-зависимости — `@evgkch/machjs >= 0.2.1` и `@evgkch/chanjs >= 1.1.0`: в сборку пакета они не входят, и копия `machjs` у приложения должна быть одна (см. [Ограничения](#ограничения)).
