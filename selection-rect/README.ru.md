...
Условия из п. 2.3 должны различать нажатие на ручку и нажатие в середину, а значит — читать сам прямоугольник. Для этого нужны геометрические типы.
...
Координаты округляются один раз, на входе: при зуме страницы и на HiDPI-экране `clientX` и ограничивающий прямоугольник дробные, а весь дальнейший счёт — контекст, допуск условий, CSS-рамки — идёт от целых значений.
...
Условие читает `source` и `target` одного перехода, поэтому запись появляется только на шаге *внутрь* перетаскивания — по одной на операцию, сколько бы событий `move` она ни заняла.
