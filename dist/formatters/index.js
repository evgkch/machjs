...
export const toMermaid = (schema, options) => {
...
/** Graphviz DOT. */
export const toDot = (schema, options) => {
 ...
 */
export const toTree = (schema, options) => {
    ...
    for (const node of options?.at !== undefined
        ...
        const name = options?.color && options?.current === node
            ? invert(String(node))
            : String(node);
